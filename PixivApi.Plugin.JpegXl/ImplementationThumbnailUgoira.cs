...
public sealed record class ImplementationThumbnailUgoira(string ExePath, ConfigSettings ConfigSettings) : IFinder, IConverter
{
    public static ValueTask<IPlugin?> CreateAsync(string dllPath, ConfigSettings configSettings, CancellationToken cancellationToken)
    {
        ...
        return ValueTask.FromResult<IPlugin?>(exePath is null ? null : new ImplementationThumbnailUgoira(exePath, configSettings));
    }
